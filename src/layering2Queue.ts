...
import { scroll3, scroll, findAgeGroup, eventsWithinPeriod } from "./utils";
...
const generateLayering = (options: {
    ...
    const { trackedEntityInstances, allSessions, periods } = options;
    let layering: any[] = [];
    ...
    for (const {
        ...
        for (const period of periods) {
            ...
            sessionsWithinQuarter.forEach((session) => {
                ...
                const currentSessions = session["ygHFm67aRqZ"].split(",");
